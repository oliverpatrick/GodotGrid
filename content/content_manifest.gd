...
func definition_by_id(definition_id: String):
	for group in [definitions.get("items", []), definitions.get("resources", []), definitions.get("npcs", []), definitions.get("drop_tables", []), definitions.get("dialogues", [])]:
		for definition in group:
			if str(definition.get("id", "")) == definition_id:
				return definition
	return null

func npc_by_id(definition_id: String):
	for definition in definitions.get("npcs", []):
		if str(definition.get("id", "")) == definition_id:
			return definition
	return null
